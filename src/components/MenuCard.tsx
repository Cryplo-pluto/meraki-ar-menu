...
export function MenuCard({ item, hrefBase }: { item: MenuItem; hrefBase: string }) {
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-border/60 bg-card warm-shadow transition hover:-translate-y-0.5 hover:border-primary/40">
      <a
        href={href}
        className="relative block aspect-[4/3] overflow-hidden bg-[var(--mint-tint)]"
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        <div className="flex items-start justify-between gap-3">
          <a href={href} className="text-base font-semibold text-foreground hover:text-primary">
            {item.name}
          </a>
        ...
        <a
          href={`${href}?ar=1`}
          className="mt-auto inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground transition hover:brightness-110"
