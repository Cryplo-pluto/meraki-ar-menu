...
export const Route = createFileRoute("/locations/$slug")({
  ...
  head: ({ params, loaderData }) => {
    ...
    if (b.address && b.address !== "Address to be confirmed") {
      jsonLd.address = { "@type": "PostalAddress", streetAddress: b.address, addressLocality: "Lusaka", addressCountry: "ZM" };
    }
    if (b.phone) jsonLd.telephone = b.phone;
    if (hoursSpec.length > 0) jsonLd.openingHoursSpecification = hoursSpec;
    if (b.lat && b.lng) jsonLd.geo = { "@type": "GeoCoordinates", latitude: b.lat, longitude: b.lng };
      ...
      <div className="grid gap-10 lg:grid-cols-[1fr_1.4fr]">
        <div>
          ...
          {hasAddress ? (
            ...
            <p className="mt-3 text-sm italic text-muted-foreground/80">Address being confirmed — please call ahead.</p>
          )}
            ...
            <>
              ...
              <ul className="mt-1 text-foreground/90">
                {hoursEntries.map(([day, hrs]) => (
                  <li key={day}><span className="capitalize">{day}</span>: {hrs}</li>
                ))}
              </ul>
            </>
          )}
