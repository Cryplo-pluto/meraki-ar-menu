...
export const Route = createFileRoute("/menu/$category/")({
  ...
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: `Our ${categoryLabel(params.category).toLowerCase()} at Meraki Cafe Lusaka. See every dish in true-scale AR before you order.`,
      },
  ...
  errorComponent: ({ error }) => (
    <p className="container-page py-24">Could not load menu: {error.message}</p>
  ),
    ...
    <div className="container-page py-12">
      ...
      {items.length === 0 ? (
        <p className="mt-8 text-muted-foreground">
          Nothing in this category right now — our specials change with the season. Check back soon
          or ask in store.
        </p>
      ) : (
